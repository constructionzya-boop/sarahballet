import { cn } from "@noema/ui";
import type { ReactNode } from "react";
import { PLACEHOLDER_WHATSAPP } from "../lib/constants";
import { whatsappHref } from "../lib/whatsapp";

export type WhatsAppCTAProps = {
  message: string;
  phone?: string;
  children?: ReactNode;
  variant?: "orange" | "green";
  size?: "md" | "lg";
  className?: string;
};

/** CTA WhatsApp — le canal n°1. Génère le deep link wa.me avec message pré-rempli. */
export function WhatsAppCTA({
  message,
  phone = PLACEHOLDER_WHATSAPP,
  children,
  variant = "orange",
  size = "md",
  className,
}: WhatsAppCTAProps) {
  const variants = {
    orange: "bg-orange text-white hover:brightness-110 focus-visible:ring-orange",
    green: "bg-[#25D366] text-white hover:brightness-105 focus-visible:ring-[#25D366]",
  };
  const sizes = { md: "h-11 px-5 text-sm", lg: "h-14 px-7 text-base" };
  return (
    <a
      href={whatsappHref(message, phone)}
      target="_blank"
      rel="noopener noreferrer"
      className={cn(
        "inline-flex items-center justify-center gap-2 rounded-full font-semibold transition-[filter] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 active:scale-95",
        variants[variant],
        sizes[size],
        className,
      )}
    >
      <svg viewBox="0 0 24 24" aria-hidden="true" className="size-5 fill-current">
        <path d="M12.04 2c-5.46 0-9.9 4.44-9.9 9.9 0 1.75.46 3.45 1.32 4.95L2 22l5.3-1.38a9.86 9.86 0 0 0 4.73 1.2h.01c5.46 0 9.9-4.44 9.9-9.9 0-2.64-1.03-5.13-2.9-7A9.82 9.82 0 0 0 12.04 2Zm5.8 14.03c-.24.68-1.4 1.3-1.94 1.35-.5.05-1.13.07-1.82-.11-.42-.13-.96-.31-1.65-.61-2.9-1.25-4.8-4.17-4.94-4.36-.15-.19-1.18-1.57-1.18-3s.75-2.13 1.02-2.42c.27-.29.58-.36.78-.36.19 0 .39 0 .56.01.18.01.42-.07.66.5.24.58.82 2.01.9 2.16.07.15.12.32.02.51-.1.19-.15.31-.29.48-.15.17-.31.38-.44.51-.15.15-.3.31-.13.6.17.29.76 1.25 1.63 2.03 1.12 1 2.06 1.31 2.35 1.46.29.15.46.12.63-.07.17-.19.73-.85.92-1.14.19-.29.39-.24.66-.15.27.1 1.7.8 1.99.95.29.15.48.22.55.34.07.12.07.68-.17 1.35Z" />
      </svg>
      {children ?? "Devis WhatsApp"}
    </a>
  );
}
